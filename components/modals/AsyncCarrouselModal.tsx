...
export default function SimpleBackdrop({
  ...
  const imagePlaceholders = unLoadedImagesCount
    ? new Array(unLoadedImagesCount).fill({ image: '', title: 'loading image' })
    : [];
  React.useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  const carouselHeight = isInSmScreen ? 450 : 600;
  ...
  const miniaturesMarginLeft =
    carouselIndex > miniaturesCenter
      ? (carouselIndex - miniaturesCenter) * miniaturesWidth
      : 0;
          ...
          >
            <ImageList
              sx={{
                ...
                transform: `translateX(-${miniaturesMarginLeft}px)`,
                my: 0,
              }}
              cols={totalImages}
              rowHeight={miniaturesHeight}
            >
              {[...images, ...imagePlaceholders].map((item, index) => (
                <ImageListItem
                  key={item.image + index}
                ...
                >
                  <Box
                    ...
                    sx={{
                      ...
                      height: miniaturesHeight,
                      backdropFilter:
                        carouselIndex === index ? 'unset' : 'brightness(0.5)',
                      cursor: 'pointer',
                    }}
                  >
                    {item.image ? (
...
function ImagePlaceholder() {
  return (
    ...
    >
      <CircularProgress sx={{ 'svg,span': { color: '#fff' } }} />
    </Box>
  );
}
