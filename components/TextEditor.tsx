...
export default function MyEditor({
  ...
  const mapKeyToEditorCommand = (e: React.KeyboardEvent<HTMLElement>) => {
    ...
    if (e.keyCode === 65 && hasCommandModifier(e))
      return toggleBlockType(e, 'header-four');
    /* `M` key  */
    if (e.keyCode === 77 && hasCommandModifier(e))
      return toggleBlockType(e, 'unordered-list-item');
    /* `Y` key  */
    if (e.keyCode === 89 && hasCommandModifier(e))
      return toggleBlockType(e, 'ordered-list-item');

    /* ` Q "` key  */
    if (e.keyCode === 81 && hasCommandModifier(e))
      return toggleBlockType(e, 'blockquote');
  ...
  const BLOCK_TYPES: { icon: React.ReactNode; style: blockStyles }[] = [
    {
      icon: <TitleIcon fontSize={isInSmScreen ? 'medium' : 'small'} />,
      style: 'header-four',
    },

    {
      icon: (
        <FormatListBulletedIcon fontSize={isInSmScreen ? 'medium' : 'small'} />
      ),
      style: 'unordered-list-item',
    },
    {
      icon: (
        <FormatListNumberedIcon fontSize={isInSmScreen ? 'medium' : 'small'} />
      ),
      style: 'ordered-list-item',
    },
    {
      icon: <FormatQuoteIcon fontSize={isInSmScreen ? 'medium' : 'small'} />,
      style: 'blockquote',
    },
  ];
  const INLINE_STYLES: { icon: React.ReactNode; style: InlineStyles }[] = [
    {
      icon: <FormatBoldIcon fontSize={isInSmScreen ? 'medium' : 'small'} />,
      style: 'BOLD',
    },
    {
      icon: <FormatItalicIcon fontSize={isInSmScreen ? 'medium' : 'small'} />,
      style: 'ITALIC',
    },
    {
      icon: (
        <FormatUnderlinedIcon fontSize={isInSmScreen ? 'medium' : 'small'} />
      ),
      style: 'UNDERLINE',
    },
  ];
    ...
    <ErrorBoundary>
      ...
      >
        {INLINE_STYLES.map((inlineStyle) => (
          <Tooltip
            title={stylesControls[inlineStyle.style]}
            key={inlineStyle.style}
          >
            <IconButton
              size={isInSmScreen ? 'medium' : 'small'}
              aria-label={inlineStyle.style}
              component="span"
              color={
                currentStyle.has(inlineStyle.style) ? 'primary' : 'default'
              }
              onClick={(e: React.MouseEvent<HTMLElement>) =>
                toggleInlineStyle(e, inlineStyle.style)
              }
            >
              {inlineStyle.icon}
            </IconButton>
        ...
        {BLOCK_TYPES.map((blockStyle) => (
          <Tooltip
            title={stylesControls[blockStyle.style]}
            key={blockStyle.style}
          >
            <IconButton
              size={isInSmScreen ? 'medium' : 'small'}
              aria-label={blockStyle.style}
              component="span"
              color={blockStyle.style === blockType ? 'primary' : 'default'}
              onClick={(e: React.MouseEvent<HTMLElement>) =>
                toggleBlockType(e, blockStyle.style)
              }
            >
              {blockStyle.icon}
            </IconButton>
