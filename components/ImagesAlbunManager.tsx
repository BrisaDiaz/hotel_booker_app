...
export default function ImageManager({defaultValue,onSubmit}:{defaultValue?:string[],onSubmit:(newFiles:File[],defaultFiles?:string[])=>void}) {
const [visuals, setVisuals] = React.useState<{url:string,name:string}[]>(defaultValue? [...defaultValue.map(url => ({name:url,url:url}))]: [])
const [files, setFiles] = React.useState<File[]>([])
const [defaultImgUrls, setDefaultImgUrls] = React.useState<string[]>(defaultValue||[])

const allowedExtensions = /(\.jpg|\.jpeg|\.png|\.gif)$/i;

const handleFilesUpload=async(e:any)=>{
    const currentFilesNames=files.map(file=>file.name)
    const nonDuplicatedFiles= [...e.target.files].filter(file=> !currentFilesNames.includes(file.name) &&allowedExtensions.exec(file.name)) 
   setFiles([...files,...nonDuplicatedFiles])

const readerPromices = nonDuplicatedFiles.map(file => {
   ...
   const newVisuals:any[]= await Promise.all(readerPromices)
 ...
 setFiles(files.filter(file=> !filesNames.includes(file.name)))
      ...
      setDefaultImgUrls(defaultImgUrls.filter(url=> !filesNames.includes(url)))
   }
 ...
 const handleResetChanges=()=>{
       setFiles([])
      setVisuals(defaultValue? [...defaultValue.map(url => ({name:url,url:url}))]: [])
