...
function ImageContainer({image,onSelect}:{image:{name:string,url:string},onSelect:(e:any)=>void}){
  ...
  <Image src={generateImageUrl(image.url,{height:170,width:270})}                  placeholder="blur"

           blurDataURL={generateImageUrl(image.url,{height:170,width:270,quality:10})}  width={160} height={100} layout="responsive" alt={image.name} 
onLoad={(e:any) => {
    e.target.src.indexOf('data:image/gif;base64') < 0 &&  setIsLoaded(true)
  }}/>
   <Box sx={{position:'absolute',width:'100%',height:'100%',zIndex:5,top:0,left:0,display:'flex',justifyContent:'center',alignItems:'center',backdropFilter:isLoaded ?'brightness(0.9)':'brightness(1)'}}>
