...
const RoomRequests: WithLayoutPage<PagePromps> = (props) => {
  const generateFolders = (albums: Partial<Album>[]) => {
    return albums.reduce(
      (folders: Folder[], album: Partial<Album>) => {
        ...
        album.roomModelId
          ? folders[1].subFolders.push(folderData)
          : folders[0].subFolders.push(folderData);
        return folders;
      },
      [
        { id: 1, name: 'Hotel Albums', subFolders: [] },
        { id: 2, name: 'Room Types Albums', subFolders: [] },
      ]
    );
  };
  const [folders, setFolders] = React.useState<Folder[]>(
    generateFolders(props.albums)
  );
  const [albumToEditImages, setAlbumToEditImages] = React.useState<
    { id: number; src: string }[] | null
  >(null);
  ...
  const [deleteAlbum, deleteAlbumRequest] = useMutation(DELETE_ALBUM, {
    onCompleted({ album }: { album: { id: number; name: string } }) {
      const foldersCopy = [...folders];
      const actualizedSubFolders = foldersCopy[0].subFolders.filter(
        (folder) => folder.id !== album.id
      );
  ...
  const [renameAlbum, renameAlbumRequest] = useMutation(RENAME_ALBUM, {
    onCompleted({ album }: { album: { id: number; name: string } }) {
      const foldersCopy = [...folders];
      const actualizedSubFolders = foldersCopy[0].subFolders.map((folder) =>
        folder.id === album.id ? album : folder
      );
      foldersCopy[0].subFolders = actualizedSubFolders;
  ...
  React.useEffect(() => {
    ...
    if (albumImagesRequest.error) {
      return notify({
        type: 'error',
        content: albumImagesRequest.error.message,
      });
    }
  }, [albumImagesRequest]);
  ...
  const handleActions = async (
    ...
    if (action === 'renameAlbum' || action === 'deleteAlbum') {
      const isHotelAlbum = folders[0].subFolders.find(
        (folder: { name: string; id: number }) =>
          selectedAlbum?.id === folder.id
      );

      if (!isHotelAlbum) {
        return notify({
          type: 'warning',
          content: 'Room types albums cannot be renamed or deleted.',
        });
      }
    }
  ...
  const onEditAlbumImgs = async (
    toUploadFiles: File[],
    imagesFiltered?: string[]
  ) => {
    if (!selectedAlbum) return;
    setLoading(true);
    try {
      const uploadedImgs = toUploadFiles.length
        ? await uploadToCloudinary(toUploadFiles)
        : [];
      let imagesUrls = uploadedImgs.length
        ? uploadedImgs.map((img) => img.secure_url)
        : [];
      if (imagesFiltered) {
  ...
  React.useEffect(() => {
    if (
      createAlbumRequest.loading ||
      albumImagesRequest.loading ||
      renameAlbumRequest.loading ||
      editAlbumImgsRequest.loading ||
      deleteAlbumRequest.loading
    ) {
  ...
  return (
    <div>
      ...
      <Box sx={{ maxWidth: 1200 }} component="main">
        ...
        <FullScreenModal
          isOpen={modalsState.manageAlbum}
          onClose={() => handleCloseModal('manageAlbum')}
          title={selectedAlbum?.name}
        >
          <ImageManager
            onSubmit={onEditAlbumImgs}
            defaultValue={
              albumToEditImages ? albumToEditImages.map((img) => img.src) : []
            }
          />
        </FullScreenModal>

        <AlbumModal
          isOpen={modalsState.createAlbum}
          onClose={() => handleCloseModal('createAlbum')}
